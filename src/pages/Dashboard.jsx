...
export default function Dashboard() {
  ...
  const { isConnected: wsActive } = useMarketSocket((live) => {
    if (lastAlertPrice > 0) {
      const percentChange = Math.abs(live.price - lastAlertPrice) / lastAlertPrice;
      if (percentChange >= 0.01) {
        dispatch(addNotification({
          type: live.price > lastAlertPrice ? "success" : "warning",
          category: "market",
          title: "Market Volatility Alert",
          message: `XLM price moved significantly to $${live.price.toFixed(4)}.`,
          walletId: activeWalletId,
        }));
        dispatch(showToast({ message: `Significant price movement detected!`, type: "info" }));
        setLastAlertPrice(live.price);
      }
    } else {
      setLastAlertPrice(live.price);
    }
    ...
    setMarketData((prev) => {
      ...
      return {
        ...prev,
        xlm: { ...prev.xlm, price: live.price, change: live.change }
      };
    });
  });
  ...
  useEffect(() => {
    const checkNetwork = async () => {
      try {
        const url = network === "public" ? "https://horizon.stellar.org" : "https://horizon-testnet.stellar.org";
        const res = await fetch(url, { signal: AbortSignal.timeout(5000) });
        setIsHorizonOnline(res.ok);
      } catch {
  ...
  const portfolioValue = marketData
    ? (parseFloat(nativeBalance) * (marketData?.xlm?.price ?? 0)).toFixed(2)
    : "0.00";
    ...
    return (
      <div className="grid lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* MARKET INSIGHTS */}
          <div className="bg-[#1e2329] p-6 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden group">
            <div className="absolute top-6 right-6 flex items-center gap-2 px-2.5 py-1 bg-black/40 rounded-full border border-white/5">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-end gap-3">
                <h1 className={`text-4xl font-black tracking-tighter transition-colors duration-500 ${marketTrend === 'UP' ? 'text-emerald-400' : marketTrend === 'DOWN' ? 'text-rose-400' : 'text-white'}`}>
                  ${marketData?.xlm?.price?.toFixed(4) ?? "0.0000"}
                </h1>
                <div className={`flex items-center gap-1 mb-1 px-2 py-0.5 rounded-md text-[10px] font-black ${ (marketData?.xlm?.change ?? 0) >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                   {(marketData?.xlm?.change ?? 0) >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownLeft size={14} />}
                   {Math.abs(marketData?.xlm?.change ?? 0).toFixed(2)}%
                </div>
              </div>
            </div>
  ...
  }, [balances, activeWallet, loading, portfolioValue, marketData, priceHistory, wsActive, marketTrend]);
  ...
  return (
    <div className="bg-[#0b0e11] min-h-screen text-white relative overflow-x-hidden">
      <ErrorBoundary>
