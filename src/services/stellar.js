...
export const sendStellarTransaction = async (
  ...
  if (!secretKey || !StrKey.isValidEd25519SecretSeed(secretKey)) {
    throw new Error("Invalid source secret key. Please ensure your wallet is unlocked.");
  }

  // ✅ STEP 2: FIX AMOUNT FORMAT (Must be string with 7 decimals)
  ...
  if (destExists) {
    ...
    throw new Error("Destination account does not exist. Custom assets cannot be sent to unfunded accounts; they must first be funded with XLM and create a trustline.");
  }

  // ✅ STEP 4: FIX TRANSACTION BUILD
