...
const getPersistedAuthState = () => {
  ...
  // If no PIN exists, the app shouldn't be "locked" in the security sense; it's in onboarding.
  const lockFlag = typeof window !== 'undefined' ? localStorage.getItem("nexa_app_locked") : null;

  // Industry Standard: App starts unlocked on load. Manual lock triggers the vault gate.
  const isAppUnlocked = true;
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase("wallet/addWallet", (state) => {
      state.hasWallet = true;
    });
    builder.addCase("wallet/trashWallet", (state, action) => {
      // We need to know if any wallets are left. 
      // Since we don't have access to wallet state here, 
      // we'll rely on the fact that trashWallet is only called when at least one wallet exists.
      // But a better way is to check localStorage or move hasWallet to walletSlice.
      const rawWallets = localStorage.getItem("nexa_wallets");
      const wallets = JSON.parse(rawWallets || "[]");
      state.hasWallet = wallets.length > 0;
    });
    builder.addCase("wallet/deleteWallet", (state, action) => {
      const rawWallets = localStorage.getItem("nexa_wallets");
      const wallets = JSON.parse(rawWallets || "[]");
      state.hasWallet = wallets.length > 0;
    });
    builder.addCase(APP_RESET, (state) => {
