...
const rootReducer = (state, action) => {
  ...
  if (action.type === APP_RESET.type) {
    console.log("🔄 Redux Root: Session Reset Activated");
...
const sessionTimeoutMiddleware = (store) => (next) => (action) => {
  ...
  if (state.auth.isWalletUnlocked) {
    ...
    if (action.type !== lockWallet.type && action.type !== APP_RESET.type) {
      // Refine: Update the Redux lastActivity timestamp to keep state in sync with real-time events
      if (action.type !== updateLastActivity.type) {
