  ...
  const startScan = async (scannerInstance) => {
    ...
    try {
      await scannerInstance.start(
        ...
        { 
          fps: 30, // Maximum fluidity
          qrbox: { width: 300, height: 300 },
          aspectRatio: 1.0 
        },
        (decodedText) => {
          const trimmed = decodedText.trim();
          if (!StrKey.isValidEd25519PublicKey(trimmed)) return;

          if (scanned) return;
          setScanned(true);
          setValidAddress(trimmed);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-[#0b0e11] overflow-hidden select-none">
      ...
      <div className="absolute inset-0 z-0">
        <div 
          id="qr-scanner-view" 
          className="w-full h-full [&>video]:object-cover [&>video]:brightness-[1.6] [&>video]:contrast-[1.4] [&>video]:saturate-[1.3] [&>video]:will-change-transform" 
        />
      </div>
